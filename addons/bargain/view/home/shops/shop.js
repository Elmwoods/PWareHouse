	...
	$.post(WST.AU('bargain://shops/pageByJoins'),params,function(data,textStatus){
	    ...
	    if(json.status==1 && json.data){
	    	json = json.data;
	       	...
	       	laytpl(gettpl).render(json.Rows, function(html){
	       		$('#list').html(html);
	       		$('.j-lazyUserImg').lazyload({ effect: "fadeIn",failurelimit : 10,skip_invisible : false,threshold: 200,placeholder:window.conf.ROOT+'/'+window.conf.USER_LOGO});
	       	});
	       	laypage({
		        cont: 'pager', 
		        pages:json.TotalPage, 
