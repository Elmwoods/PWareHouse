...
class Distribut extends Base{
	...
	public function uninstallMenu(){
		...
		try{
			...
			Db::name('privileges')->where(["privilegeCode"=>array("like","DISTRIBUT_%")])->delete();
	...
	public function queryDistributMoneys(){
		...
		if($userName!=""){
			$where["o.userName|o.loginName"] = array("like",$orderNo."%");;
		}
	...
	public function queryDistributGoods(){
		...
		foreach ($rs['Rows'] as $key => $v){
			...
			$rs['Rows'][$key]['distributType'] = $shopConf["distributType"];
			$rs['Rows'][$key]['distributType'] = $shopConf["distributType"];
			$rs['Rows'][$key]['distributOrderRate'] = $shopConf["distributOrderRate"];
		}
	...
	public function setOrderDistribut($orderId){
		...
		$order = Db::name('orders')->where("orderId",$orderId)->field("shopId,goodsMoney,userId")->find();
		$shopId = $order["shopId"];
		$userId = $order["userId"];
		...
		if($conf["isDistribut"]==1 && $parentId>0){
			...
			if($conf['distributType']==1){//按商品提成佣金
				...
				$data["totalCommission"] = round($order["goodsMoney"]*$conf["distributOrderRate"]/100,2);
			}
		}else{
	...
	public function queryMineUsers(){
		...
		$where = array();
		$where["parentId"] = array("in",implode(",",$userIds));
		...
		foreach ($rs['Rows'] as $key => $v){
			$rs['Rows'][$key]['userCnt'] = isset($ulist[$v["userId"]])?$ulist[$v["userId"]]:0;
		}
		return $rs;
	}
	...
	public function queryAdminDistributShops(){
		...
		foreach ($rs['Rows'] as $key => $v){
			$rs['Rows'][$key]['thirdRate'] = $thirdRate."%";
			$rs['Rows'][$key]['secondRate'] = $secondRate."%";
			$rs['Rows'][$key]['buyerRate'] = $buyerRate."%";
		}
		return $rs;
	}
	...
	public function queryAdminDistributMoneys(){
		...
		$rs = Db::name('distribut_moneys dm')
				...
				->paginate(input('pagesize/d'))->toArray();
		foreach ($rs['Rows'] as $key => $v){
			$rs['Rows'][$key]['userName'] = $rs['Rows'][$key]['userName']!=""?$rs['Rows'][$key]['userName']:$rs['Rows'][$key]['loginName'];
		}
		return $rs;
	}
	...
	public function getUserInfo(){
		...
		$user['ranks'] = Db::name('user_ranks')->where(['dataFlag'=>1])->where('startScore','<=',$user['userTotalScore'])->where('endScore','>=',$user['userTotalScore'])->field('rankId,rankName,rebate,userrankImg')->find();
