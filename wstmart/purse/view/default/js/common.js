...
//Ajax刷新获取的条数
WST.news_count = function(){
	 $.get(WST.U('purse/Consume/news_count'),{user_name:a},function(data){
	 	$('#news_count').html(data);
	 });
...
//金豆余额查询
WST.balance = function(){
	$.post(WST.U('purse/Predeposit/balance'),{imazamox_number:a},function(data){
		if(data == 'success'){
			// alert('成功')
			// $('#yan').show();
...
WST.Payment = function(){
	$.post(WST.U('purse/Predeposit/Payment'),{userMoney:y},function(data){
...
WST.Other = function(){
	// alert(u);
	$.post(WST.U('purse/Predeposit/Other'),{loginName:u},function(data){
		if(data=='success'){
			alert('存在');
		}else{
			alert('不存在');
...
//余额查询
 WST.With = function(){
 	$.post(WST.U('purse/Predeposit/With'),{userMoney:va},function(data){
 ...
 WST.Verification = function(){
 	$.post(WST.U('purse/Predeposit/Verification'),{loginName:va,userPhone:po},function(data){
		// alert(data);
		if(data=='success'){
			// alert('成功');
			$('#sub').attr("disabled",false);
...
//剩余还款
WST.Loan = function(){
	$.post(WST.U('purse/Predeposit/Loan'),{Money:va},function(data){
...
//我的金豆ajax删除
WST.del_jd = function(id){
	WST.confirm({content:'您确定要删除该信息？',yes:function(index){
		$.post(WST.U('purse/Consume/del_jd'),{id:id},function(data){
			var json = WST.toJson(data);
			if(json.status==1){
				WST.msg(json.msg,{icon:1});
				location.href=WST.U('purse/Imazamox/index');
			}else{
				WST.msg(json.msg,{icon:2});
			}
		});
...
//我的贷款ajax删除
WST.del_dl = function(id){
	WST.confirm({content:'您确定要删除该信息？',yes:function(index){
		$.post(WST.U('purse/Consume/del_dl'),{id:id},function(data){
			var json = WST.toJson(data);
			if(json.status==1){
				WST.msg(json.msg,{icon:1});
