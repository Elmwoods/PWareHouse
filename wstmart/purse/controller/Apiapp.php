	...
	class Apiapp {
		...
		public function bill(){
			...
			if(!empty($_POST['id']) && !empty($_POST['token']) && $_POST['token']!=null && $_POST['id']!=null){
		        ...
		        if(isset($_POST['type'])&&($_POST['type']!=0)&&($_POST['type']!=7))
		...
		public function imazamox_index($id){
			...
			if(!empty($_POST) && $_POST['id'] != null && $_POST['token'] != null){
				if(self::token($_POST['id'],$_POST['token'])==='success'){
			        ...
			        ->where('dataId','in','10,11,12,13,18,20，22')
			        ->where('dataFlag','not in','-1')
			        ->order('id desc')
			        ->paginate(10);
			        // echo '<pre>';
		                                ...
		                                try{
		                                    ...
		                                    if($me==1&&$mem==1){
		                                    	$data = array(
		                                        	...
		                                        	'remark'       =>   textEncode(htmlspecialchars($_POST['desc'])),
						...
						if($_POST['member_name'] != $users['loginName'] && $_POST['member_name'] != $users['userPhone']) {
				            if (is_numeric($_POST['amount']) && $_POST['amount'] >= '0.01') {
			                    ...
			                    try{
			                        ...
			                        $data['msgTitle']      =   '收款';
			                        $data['msgContent']    =   '用户' . $users['loginName'] . '向'.$user['loginName'].'收款' . $_POST['amount'] . '元，说明：' . textEncode(htmlspecialchars($_POST['desc']));
			                        $data['amount']        =   $_POST['amount'];
			                        $data['tradeNo']       =   $tradeNo;
			                        $data['dataFlag']      =   0;
			                        $data['msgDesc']       =   textEncode(htmlspecialchars($_POST['desc']));
			                    ...
			                    if ($news) {
				                    ...
				                    $insert['payType']     =   1;
				                    $insert['createTime']  =   date('Y-m-d H:i:s');
				                    $insert['remark']      =   textEncode(htmlspecialchars($_POST['desc']));
				                    $in = \think\Db::name('log_moneys')->insert($insert);
			...
			if (self::token($_POST['id'],$_POST['token'])==='success') {
				...
				if(is_numeric($_POST['number'])&&$user){
					...
					if($user['userMoney'] >= $_POST['number'] && $_POST['number'] >= '0.01'){
						...
						->update(['userMoney'=>$user['userMoney']-sprintf('%.2f',$_POST['number']),
								  'imazamox_number'=>$user['imazamox_number']+sprintf('%.2f',$_POST['number'])]);	
						if($users){
