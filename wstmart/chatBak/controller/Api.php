...
class Api extends Controller {
                ...
                if ($arr2) {        //如果已注册,则返回token和token_id
                    ...
                    if ($id) {      //如果插入成功,则往下执行
                        if (mb_substr(strval($id), -2) == '00') {       //判断末两位是否为00(凡是末两位为00的均为客服人员)
                            ...
                            $users -> startTrans();
                            //同时插入客服和新注册用户两条数据
                            if ($users -> insert(['userId'=>$id, 'loginName'=>'server'.mb_substr($id, 0, -2 ), 'userName'=>'客服'.mb_substr($id, 0, -2 ),'loginSecret'=>$loginSecret, 'loginPwd'=>md5('12345678'.$loginSecret), 'createTime'=>date('Y-m-d H:i:s', time())]) && $id2 = $users -> insertGetId($values)) {
                                $users -> commit();
                            }else {
                                $users -> rollback();
                                $data = array(
    ...
    private function getUserInfo($token_id, $token) {
        $users = db('users a');
        $data = array('a.userId', 'a.loginName', 'a.trueName', 'a.userPhoto', 'a.userPhone', 'a.userSex', 'a.brithday', 'a.userAge', 'a.signature', 'a.userEmail', 'a.userName', 'a.userNation', 'a.userProvince', 'a.userCity', 'a.setAge', 'a.setDistance', 'a.isValidate', 'a.setStranger', 'a.isRec', 'a.isSeted');
