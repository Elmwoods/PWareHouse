...
use wstmart\common\model\Sdk as S;
	...
	class Apiapp {
		...
		// token 身份验证
		protected function token($id,$token) {
		...
		public function loan_index($id) {
			...
			if(!empty($_POST) && $_POST['token'] != null && $_POST['token'] != null){
				if(self::token($_POST['id'],$_POST['token'])==='success'){
					...
					->field('residual_repayment,userScore,repayment_date')
					->find();
		...
		public function loan($id){
			...
			if(!empty($_POST) && $_POST['id'] != null && $_POST['token'] != null){
				...
				if(self::token($_POST['id'],$_POST['token'])==='success'){
			        ...
			        ->where(['userId' => $id])
			        ->field('repayment_date,residual_repayment,userMoney,userType,userId,userScore')
			        ->find();
			// dump($user['userType']);
		...
		public function repayment($id){
			...
			//        var_dump($_POST);die;
			        ...
			        $user = \think\Db::name('users')
			        ->where(['userId' => (int)$id])
			        ->field('residual_repayment,userMoney,userId,userType')
			        ->find();
			       // dump($user);
			...
			if(!empty($_POST) && $_POST['id'] != null && $_POST['token'] != null){
				if(self::token($_POST['id'],$_POST['token'])==='success'){
		        	...
		        	if($im){
			        	...
			        	return json_encode(array('info'=>'error','money'=>null),JSON_UNESCAPED_UNICODE|JSON_PRETTY_PRINT);
		        	}
			        // echo '</pre>';
				}else{
		       		return json_encode(array('resultcode'=>$s->token_ne(),'info'=>'error'),JSON_UNESCAPED_UNICODE|JSON_PRETTY_PRINT);
   ...
   //@param dataId 8贷款 9还款
    public function loan_record(){
    	...
    	if(!empty($_POST) && $_POST['id'] != null && $_POST['token'] !=null){
	    	if(self::token($_POST['id'],$_POST['token'])==='success'){
	    		//只有贷款没有还款的查询
		    	$al = \think\Db::name('log_moneys')->where('targetId',(int)$_POST['id'])->where('dataId','in','8,9')->order('id desc')->paginate(10);
		    	// return 
		    	if($al){
		    		//添加星期 
		    		foreach ($al as $k => $v) {
    	...
    	// if(!empty($_POST)){
		    	...
		    	if(self::token($_POST['id'],$_POST['token'])==='success'){
			    	...
			    	// if($user){
				    	...
				    	$al = \think\Db::name('log_moneys')->where('targetId',$_POST['id'])->where('dataId','in','4,5,6,7,8,9,14,15,16')->order('id desc')->paginate(10);
				    	foreach ($al as $k => $v) {
						        	$v['monday'] = date('w',strtotime($v['createTime']));
						        	$al[$k] = $v;
					        	}
    ...
    public function imazamox(){
    	...
    	if(!empty($_POST) && $_POST['id'] != null && $_POST['token'] !=null){
	    	if(self::token($_POST['id'],$_POST['token'])==='success'){
		    	...
		    	->field('userId,imazamox_number,userType,loginName')
		    	->find();
		    	if($users){
		    		//本人
	...
	public function imazamox_new(){
		...
		if(!empty($_POST) && $_POST['id'] != null && $_POST['token'] != null){
			if(self::token($_POST['id'],$_POST['token'])==='success'){
					...
					$users = \think\Db::name('users')
					->where('userId',(int)$_POST['id'])
					->field('userId,loginName,userPhone,userType')
					->find();
	...
	public function imazamox_Recharge(){
		...
		if(!empty($_POST) && $_POST['id'] != null && $_POST['token'] !=null){
			if (self::token($_POST['id'],$_POST['token'])==='success') {
				...
				$user = \think\Db::name('users')
				->field('userMoney,imazamox_number,userId,userType,loginName')
				->where('userId',(int)$_POST['id'])
				->find();

				if(is_numeric($_POST['number'])){
					// dump($user['userMoney']);die;
					if($user['userMoney'] >= $_POST['number'] && $_POST['number'] >= '0.1'){
						...
						if($users){
							...
							return json_encode(array('resultcode'=>$s->imazamox_czpurse_ok(),'info'=>'success'),JSON_UNESCAPED_UNICODE|JSON_PRETTY_PRINT);
						}else{
							//失败信息添加
							$data1 = array(
			                   		'targetType' =>   '',
	        ...
	        if(self::token($_POST['id'],$_POST['token'])==='success'){
	        	...
	        	if(is_numeric($_POST['amount'])){
	        		if($user['imazamox_number']>=$_POST['amount']){
			        	...
			        	if($m){
			        		// echo 1;
	    					return json_encode(array('resultcode'=>$s->Cash_ok(),'info'=>'success'),JSON_UNESCAPED_UNICODE|JSON_PRETTY_PRINT);	
			        	}else{
			        				//流水表记录//失败记录
			        				$data1 = array(
			                   		'targetType' =>   $user['userType'],
   ...
   public function repayment_details(){
	...
	public function friendsList(){
		...
		if(!empty($_POST) && $_POST['id'] != null && $_POST['token'] != null){
			if(self::token($_POST['id'],$_POST['token'])==='success'){
				...
				->field('b.userPhoto,b.userName,a.friendId,b.userPhone,a.nickName,b.loginName')
				->where('a.userId',(int)$_POST['id'])
		                    ...
		                    if ($user['userMoney'] >= $_POST['amount']) {
		                        if ($users) {
		                                ...
		                                try{
		                                    ...
		                                    return json_encode(array('resultcode'=>$s->code_purse_ok(),'info'=>'success','order'=>$tradeNo),JSON_UNESCAPED_UNICODE|JSON_PRETTY_PRINT);
	...
	public function givefriendsRecharge(){
		...
		//充值账号用户查询 name
 		...
 		$user = \think\Db::name('users')
 		->where('userId',(int)$id)
 		->field('userId,userType,userMoney,loginName')
 		->find();
 		//判断是不是好友
 		// $fri = \think\Db::name('f')
 		...
 		if($user['userMoney']>=input('post.amount')){
            ...
            try{
 			...
 			->where('loginName|userPhoto',input('post.name'))
 			...
 			return json_encode(array('resultcode'=>transfer_purse_monery(),'info'=>'error'));
 		}
 		// 	}else{
		//     	return json_encode(array('resultcode'=>$s->token_ne(),'resule'=>'error'),JSON_UNESCAPED_UNICODE|JSON_PRETTY_PRINT);
		// 	}
