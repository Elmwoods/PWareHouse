		...
		if($params['app_id'] == '2017011705151664' && $params['trade_status'] == 'TRADE_SUCCESS'){
			if($params['subject']=='钱袋充值'){
				...
				if($jd && $jd['imr_amount']==$params['total_amount']){
					...
					//修改用户金豆数量
					$users=\think\Db::name('users')->where('userId',$user['userId'])->update(['imazamox_number'=>$user['imazamox_number']+$params['total_amount']]);
					//将数据添加到金豆变更表
				...
				if($fri && $fri['imr_amount']==$params['total_amount']){
					//查询被充值金豆用户资料
