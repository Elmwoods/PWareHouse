...
class Group extends Controller {
    ...
    public function test() {
        $info = $this->RongCloud->creates([1,2,3], 1, '第一个群');
        var_dump( json_decode($info) -> code);
    }

    //创建群
    public function create($adminID, $name, $groupMembers) {
        ...
        if (!empty($param)) {      //判断是否post发送userId
            ...
            array_push($members,$param['adminID']);
            $values = array();
            if ($groupID) {
    ...
    public function remove($groupID, $adminID, $userId) {
        ...
        if (!empty($param)) {
            ...
            if ($user_group->where('adminID', $param['adminID'])->where('groupID', $param['groupID'])->find()) {
                //if ($arr) {
                if ($group_member->where('userId', $param['userId'])->where('groupID', $param['groupID'])->delete()) {
    ...
    //拉好友进群
    public function add($groupID, $userId, $targetIDw, $name) {
        $param = $this->request->param();
            ...
            if (Db::table('jingo_user_groupmembers') -> where($map) -> find()) {        //判断是否为群内成员
                if (count($targetID) > 1) {                                             //判断目标ID是否为多个
                    if ($groupMember -> saveAll($data)) {           //添加好友进群
                        $info = $this->RongCloud->joins($targetID, $param['groupID'], $param['name']);
                        if (json_decode($info) -> code == 200) {
    ...
    public function setQRcode($groupID, $userId) {
        ...
        if (file_exists(ROOT_PATH.'upload/qrcode/group/'.$date.'/'.$name.'.png') && Db::table('jingo_user_groupmembers') -> where($map) -> update(['qrcode'=>$path])) {
            return json(array('result'=>'success', 'value'=>'已生成群二维码'));
        }else {
            return json(array('result'=>'error', 'value'=>'生成群二维码失败,请稍候重试'));
        }
