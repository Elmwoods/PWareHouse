...
class Group extends Controller {
        ...
        if (!empty($param['groupID'])) {
            if (Db::table('jingo_user_groups')->where('groupID', $param['groupID'])->where('adminID', $param['userId'])->find()) {        //判断是否为群主
                $data = Db::table('jingo_user_groups a')
                        ->field(['a.groupID', 'a.name', 'a.icon', 'a.isJoin', 'a.adminID', 'b.groupNickName', 'b.groupName', 'b.isDisturb', 'b.isTop', 'b.isSave', 'b.groupPhoto'])
                        ->join('jingo_user_groupmembers b', 'a.groupID = b.groupID', 'left')
                        ->where(['a.groupID' => $param['groupID'], 'b.userId' => $param['userId']])
                        ->find();
                $arr = Db::table('jingo_group_notice a') -> field(['b.groupNickName', 'b.groupPhoto', 'a.content', 'a.time', 'c.userName']) -> join('jingo_user_groupmembers b', ['a.groupID = b.groupID', 'a.userId = b.userId'], 'LEFT') -> join('jingo_users c', 'a.userId = c.userId', 'LEFT') -> where(['a.groupID'=>$param['groupID']]) -> where('a.userId', $param['userId']) -> find();
                if ($arr) {
                    $data['notice'] = $arr;
                }else {
    ...
    public function members($userId, $groupID) {
        ...
        if (!empty($param)) {
            ...
            $adminID = Db::table('jingo_user_groups') -> field('adminID') -> where('groupID', $param['groupID']) -> find();
            if (Db::table('jingo_user_groupmembers') -> where(['userId'=>$param['userId'], 'groupID'=>$param['groupID']]) -> find()) {
                ...
                foreach ($data as $k => $v) {
                    ...
                    if ($data[$k]['userId'] == $adminID['adminID']) {
                        array_unshift($data2, $data[$k]);
                    }else {
                        array_push($data2, $data[$k]);
                    }
                }
                return json(array('result' => 'success', 'value' => $data2));
            }else {
    ...
    public function changeAdmin($groupID, $userId, $targetID)
    {
        ...
        if (!empty($param)) {
            if (Db::table('jingo_user_groups')->where('groupID', $param['groupID'])->where('adminID', $param['userId'])->find()) {        //判断是否为群主
                $info = Db::table('jingo_user_groupmembers a')->field(['a.groupNickName', 'b.userName'])->join('jingo_users b', 'b.userId = a.userId', 'left')->where(['a.userId'=>$param['targetID'], 'a.groupID'=>$param['groupID']])->find();
                if ($info['groupNickName']) {
                    $name = $info['groupNickName'];
                }else {
                    $name = $info['userName'];
                }
                $content = '{"message":"'.$name.'已成为新群主"}';
                $info1 = $this->RongCloud->publishGroups($param['targetID'], $param['groupID'], 'RC:InfoNtf', $content, '', '', 1, 1, 1);
                Db::startTrans();
                Db::table('jingo_user_groups')->where('groupID', $param['groupID'])->update(['adminID' => $param['targetID']]);
                if (json_decode($info1)->code == 200) {
                    Db::commit();
