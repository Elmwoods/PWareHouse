        ...
        if (!empty($param)) {
            ...
            foreach ($info as $k => $v) {
                ...
                if ($v['fromUserId'] == $param['userId']) {
                    ...
                    $data2 = Db::table('jingo_users a') -> field(['a.userPhoto', 'a.userName', 'b.nickName']) -> join('jingo_user_friends b', 'a.userId = b.friendID', 'LEFT') -> where(['b.userId'=>$param['userId'], 'b.friendID'=>$v['fromUserId']]) -> find();
                    $info[$k]['userPhoto'] = $data2['userPhoto'];
                    $info[$k]['nickName'] = $data2['nickName'];
                    $info[$k]['userName'] = $data2['userName'];
                    $info[$k]['isMe'] = '0';
                }
                /*if (in_array($v['fromUserId'], $unable)) {
                    $info[$k]['isShow'] = '0';      //是否显示 (0否1是)
