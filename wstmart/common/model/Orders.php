...
class Orders extends Base{
	...
	public function quickSubmit($orderSrc = 0, $uId=0){
		...
		$orderScoreMap = [];
		$scoreMoney = $this->getOrderScoreMoney($isUseScore,$useScore);
		//生成订单
		Db::startTrans();
		try{
			//提交订单前执行钩子
			...
			if(false !== $result){
				...
				if($order['needPay']==0){
		            ...
		            if($tpl['tplContent']!=''){
		                $find = ['${ORDER_NO}'];
		                $replace = [$orderNo];
		                WSTSendMsg($shopOrder['userId'],str_replace($find,$replace,$tpl['tplContent']),['from'=>1,'dataId'=>$orderId]);
		            }
		            //微信消息
		            if((int)WSTConf('CONF.wxenabled')==1){
	                    ...
	                    WSTWxMessage(['CODE'=>'WX_ORDER_SUBMIT','userId'=>$shopOrder['userId'],'URL'=>Url('wechat/orders/sellerorder','',true,true),'params'=>$params]);
		            } 
		        }
			}
			//删除session的购物车商品
	...
	public function shopOrdersByPage($orderStatus, $sId=0){
		...
		$page = $this->alias('o')->where($where)
		      ...
		      ->field('o.orderId,orderNo,goodsMoney,totalMoney,realTotalMoney,orderStatus,deliverType,deliverMoney,isAppraise,isRefund,o.deliverType deliverTypes
	...
	public function cancel($uId=0){
		...
		$order = $this->alias('o')->join('__SHOPS__ s','o.shopId=s.shopId','left')
		              ...
		              ->field('o.orderId,o.orderNo,s.userId,o.orderCode,o.orderType,o.payType,o.orderStatus,o.useScore,o.scoreMoney,o.realTotalMoney')->find();
		...
		if(!empty($order)){
		    ...
		    try{
				...
				if(false != $result){
                    //未付款状态则直接退回积分
                    if($order['orderStatus'] == -2 && $order['useScore']>0){
                    	$score = [];
						$score['userId'] = $userId;
						$score['score'] = $order['useScore'];
						$score['dataSrc'] = 1;
	...
	public function reject($uId=0){
		...
		$order = $this->alias('o')->join('__SHOPS__ s','o.shopId=s.shopId','left')
		              ->where(['o.userId'=>$userId,'o.orderId'=>$orderId,'o.orderStatus'=>1])
		              ->field('o.orderId,o.orderNo,s.userId,payType,o.userAddress,o.userName,o.realTotalMoney,o.scoreMoney')->find();
		    ...
		    try{
				...
				if(false != $result){
		            ...
		            if((int)WSTConf('CONF.wxenabled')==1){
	                    ...
	                    WSTWxMessage(['CODE'=>'WX_ORDER_REJECT','userId'=>$order['userId'],'URL'=>Url('wechat/orders/sellerorder','',true,true),'params'=>$params]);
		            } 
					Db::commit();
					return WSTReturn('操作成功',1);
				}
	...
	public function getMoneyByOrder($orderId = 0){
		...
		return $this->where('orderId',$orderId)->field('orderId,goodsMoney,deliverMoney,useScore,scoreMoney,totalMoney,realTotalMoney')->find();
	}
	...
	*/
	...
	public function handleVirtualGoods($orderId){
		$order= Db::name('orders')->alias('o')->join('__SHOPS__ s','o.shopId=s.shopId ','inner')
				->where('orderId',$orderId)->field('orderId,orderNo,s.userId,o.userId ouserId,o.realTotalMoney,o.payFrom')
				->find();
		//新增订单日志
		$logOrder = [];
			...
			if($needPay>0 && false != $rs){
				foreach ($orders as $key =>$v){
	                ...
	                if($v['orderType']==1){
						...
						WSTSendMsg($shop["userId"],$msgContent,['from'=>1,'dataId'=>$orderId]);
						//微信消息
					    if((int)WSTConf('CONF.wxenabled')==1){
						    $params = [];
            ...
            foreach ($orders as $order) {
                ...
                if(false != $result){
	                ...
	                if($order->orderType==1){
	                    ...
	                    //发送一条商家信息
						$msgContent = "订单【".$order->orderNo."】用户已支付完成，请尽快发货哦~";
