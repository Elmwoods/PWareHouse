...
function initGrid(){
	grid = $("#maingrid").ligerGrid({
        ...
        columns: [
	        ...
	        { display: '等级', name: 'rebate', width:80,isSort: false},
	        { display: '注册时间', name: 'createTime', width:140,isSort: false},
	        { display: '状态', name: 'userStatus', width:50,isSort: false, render:function(rowdata, rowindex, value){
	        	return (value==1)?'启用':'停用';
	        }},
	        { display: '操作', name: 'op',isSort: false,width:180,render: function (rowdata, rowindex, value){
	            var h = "";
              ...
              h += "<a href='"+WST.U('admin/userscores/touserscores','id='+rowdata['userId'])+"'>积分</a> ";
              h += "<br/><a href='"+WST.U('admin/logmoneys/tologmoneys','id='+rowdata['userId'])+"&type=0'>用户资金</a> ";
              if(rowdata['userType']==1){
                  h += "<a href='"+WST.U('admin/logmoneys/tologmoneys','id='+rowdata['shopId'])+"&type=1'>商家资金</a> ";
              }
	            return h;
	        }}
        ]
