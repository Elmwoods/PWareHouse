$(function(){
	WST.upload({
		  k:"weixinCode",
	  	  pick:"#weixinCodePicker",
	  	  formData: {dir:'sysconfigs'},
	  	  accept: {extensions: 'gif,jpg,jpeg,png',mimeTypes: 'image/jpg,image/jpeg,image/png,image/gif'},
	  	  callback:function(f){
	  		  var json = WST.toAdminJson(f);
	  		  if(json.status==1){
	  			 $('#weixinCodeMsg').empty().hide();
	  			 $('#weixinCodePrevw').empty();
	  			 $('#weixinCodePrevw').html('<img src="'+WST.conf.ROOT+'/'+json.savePath+json.name+'" width="120" hiegth="120"/>');
	  			 $('#weixinCode').val(json.savePath+json.name);
	  		  }
		  },
