...
function initGrid(){
	grid = $('#maingrid').WSTGridTree({
        ...
        rownumbers:true,
        columns: [
	        { display: '分类名称', name: 'menuName', id:'menuId', align: 'left',isSort: false},
	        { display: '页面地址', name: 'menuUrl',width: 500,isSort: false,
        	render: function (rowdata){
	            var m = '<div style="width:500px;word-wrap: break-word;padding:6px;">'+rowdata.menuUrl+'</div>';
	            return m;
        	}},
	        { display: '类型', name: 'type',width: 150,isSort: false,
	        	render: function (rowdata){
	        ...
	        { display: '排序号', name: 'menuSort',width: 100,isSort: false},
	        { display: '操作', name: 'op',width: 150,isSort: false,
	        	render: function (rowdata){
		            var h = "";
			        if(WST.GRANT.WX_ZDYCD_01)if(rowdata['parentId']==0)h += "<a href='javascript:toEdit("+rowdata["menuId"]+",0)'>新增子菜单</a> ";
		            if(WST.GRANT.WX_ZDYCD_02)h += "<a href='javascript:toEdit("+rowdata["parentId"]+","+rowdata["menuId"]+")'>修改</a> ";
		            if(WST.GRANT.WX_ZDYCD_03)h += "<a href='javascript:toDel("+rowdata["parentId"]+","+rowdata["menuId"]+")'>删除</a> ";
		            return h;
	        	}}
        ]
    });
