	...
	public function rsaCheckV1($params, $rsaPublicKeyFilePath,$signType='RSA') {
		...
		if($params['app_id'] == '2017011705151664' && $params['trade_status'] == 'TRADE_SUCCESS' ){
			if($params['subject']=='钱袋充值'){
			    $qd = \think\Db::name('pd_recharge')
				->where('pdr_sn',$params['out_trade_no'])
				->find();
			    $user=\think\Db::name('users')
			    ->where('userId',$qd['pdr_user_id'])
			    ->field('userType,userId,userMoney,imazamox_number,residual_repayment')
			    ->find();
				if($qd && $qd['pdr_amount']==$params['total_amount'] && $qd['pdr_payment_state']==0){
					//修改用户信息
					$users = \think\Db::name('users')
					->where('userId',$user['userId'])
					->update(['userMoney'=>$user['userMoney']+$params['total_amount']]);
					//修改钱袋流水
					$re=\think\Db::name('pd_recharge')
					->where('pdr_sn',$params['out_trade_no'])
					->update(['pdr_payment_state'=>1,'pdr_payment_time'=>date('Y-m-d H:i:s')]);
					//添加流水信息
					...
					$insert['dataSrc'] = '充值';
					$insert['moneyType'] = 1;
					$insert['money'] =$params['total_amount'];
					$insert['tradeNo'] = $params['out_trade_no'];
					$insert['payType'] = 1;
					$insert['Headportrait']  =   htmlspecialchars('upload/users/2016-10/20170603155425.png');
				...
				// $id  \think\Db::name('im_recharge')->where('imr_sn',$params['out_trade_no'])
				$jd=\think\Db::name('im_recharge')->where('imr_sn',$params['out_trade_no'])->find();
				if($jd && $jd['imr_amount']==$params['total_amount'] && $jd['imr_payment_state']==0){
					$user=\think\Db::name('users')->where('userId',$jd['imr_user_id'])->field('userId,userType,imazamox_number')->find();
					//修改信息
					$me=\think\Db::name('im_recharge')
					->where('imr_sn',$params['out_trade_no'])
					->update(['imr_payment_state'=>1,'imr_payment_time'=>date('Y-m-d H:i:s')]);
					if($me){
						//将数据添加到金豆变更表
						$insert['user_id']           =     $user['userId'];
						$insert['imazamox_sn']       =     $params['out_trade_no'];
						$insert['imazamox_type']     =     '充值';
						$insert['imazamox_number']   =     $params['total_amount'];
						$insert['imazamox_change']   =     $params['total_amount'];
						...
						$ins['dataSrc']              =     '金豆充值';
						$ins['moneyType']            =     1;
						$ins['money']                =     $params['total_amount'];
						$ins['tradeNo']              =     $params['out_trade_no'];
						$ins['payType']              =     1;
						$ins['Headportrait']         =     htmlspecialchars('upload/users/2016-10/20170603155425.png');
						$ins['createTime']           =     $params['gmt_create'];
						$ins['remark']               =     '金豆充值';
						$ins['endTime'] =date('Y-m-d H:i:s');
						$ins = \think\Db::name('log_moneys')->insert($ins);
						$in = \think\Db::name('imazamox_flow_log')->insert($insert);
						//修改用户金豆数量
						$users=\think\Db::name('users')->where('userId',$user['userId'])
					   ->update(['imazamox_number'=>$user['imazamox_number']+$params['total_amount']]);
					   return $params['trade_status'];
					}	
				}
			}elseif($params['subject']=='金购中国还款'){
				$hy = \think\Db::name('also_loan')->where('al_sn',$params['out_trade_no'])->find();
				if($hy && $hy['al_amonut']==$params['total_amount'] && $hy['al_status']=='待还款'){
					$user=\think\Db::name('users')->where('userId',$hy['user_id'])->field('userId,userType,residual_repayment')->find();
					//修改al_amount参数
					$al = \think\Db::name('also_loan')->where('al_sn',$params['out_trade_no'])->update(['al_status'=>'还款成功']);
					//判断是否还清贷款
					if($user['residual_repayment']-$params['total_amount']==0){
						$insert['isrepayment']=1;
					}else{
					...
					if($al){
			            ...
			            $insert['tradeNo']       =   $params['out_trade_no'];
			            $insert['payType']       =   1;
			            $insert['createTime']    =   $params['gmt_create'];
		                ...
		                $da1['msgTitle']		 =   '还款';
		                $da1['msgContent']		 =	 '还款成功';
		                $da1['amount']			 =   $params['total_amount'];
		                $da1['msgDesc']			 =	 '支付宝';	
		                $da1['tradeNo']			 =	 $params['out_trade_no'];
						...
						//修改信息
						$u=\think\Db::name('users')->where('userId',$user['userId'])->update(['residual_repayment'=>$user['residual_repayment']-$params['total_amount']]);
						return $params['trade_status'];
					}	
				}
			}elseif($params['subject']=='给好友充值金豆'){
				$fri=\think\Db::name('im_recharge')->where('imr_sn',$params['out_trade_no'])->find();
				if($fri && $fri['imr_amount']==$params['total_amount'] && $fri['imr_payment_state']==0){
					//查询被充值金豆用户资料
					$user=\think\Db::name('users')->where('userId',$fri['imr_user_id'])->field('userId,userType,imazamox_number,loginName,userPhoto')->find();
					//查询出出钱的账户资料
					$users=\think\Db::name('users')->where('userId',$fri['imr_yourself'])->field('loginName,userType,userPhoto,userId')->find();
					//修改金豆数量
					\think\Db::name('users')->where('userId',$fri['imr_user_id'])->update(['imazamox_number'=>$user['imazamox_number']+$params['total_amount']]);
					//修改im_recharge 状态
					$im=\think\Db::name('im_recharge')->where('imr_sn',$params['out_trade_no'])->update(['imr_payment_state'=>1,'imr_payment_time'=>date('Y-m-d H:s:i')]);
					//流水记录
					$data = array(//支出
