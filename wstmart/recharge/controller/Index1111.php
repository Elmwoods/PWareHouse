		...
		if(!empty($_POST) && $_POST['id'] != null && $_POST['token'] != null){
			if($this->token($_POST['id'],$_POST['token'])==='success'){
				if($_POST['purse']=='qd_success'){
					...
					// dump($recharge);
					if($_POST['amount'] ==$recharge['pdr_amount'] && $recharge['pdr_payment_state']==0 ){//判断金额或者是否充值成功
						...
						//修改钱袋流水表信息	
						$re=\think\Db::name('pd_recharge')->where('pdr_id',(int)$recharge['pdr_id'])->update(['pdr_payment_state'=>1,'pdr_payment_time'=>date('Y-m-d H:i:s')]);
						if($re){
					...
					// dump($im);
					if($im['imr_payment_state']==0 && $im['imr_amount']==$_POST['amount']){
						// echo 1;
						...
						//修改信息
						$me = \think\Db::name('im_recharge')->where('imr_id',(int)$im['imr_id'])->update(['imr_payment_state'=>1,'imr_payment_time'=>date('Y-m-d H:i:s')]);
						if($me){
							//流水表数据
							//将数据添加到金豆变更表
					...
					if($also['al_amonut'] == $_POST['amount'] && $also['al_status'] == '待还款'){
						...
						// dump($also['al_status']);die;
						//修改al_amount参数
						$al = \think\Db::name('also_loan')->where('id',(int)$also['id'])->update(['al_status'=>'还款成功']);
						// $u=\think\Db::name('also_loan')->where('id',$also['id'])->select();
						// dump($al);
						if($al){
							//修该users 表
							//将信息添加到资金流水
