...
class Chatroom{
    ...
    public function queryWhiteListUser($chatroomId) {
        try{
            ...
            $ret = $this->SendRequest->curl('/chatroom/user/whitelist/query.json',$params,'urlencoded','im','POST');
            if(empty($ret))
                throw new \Exception('bad request');
            return $ret;
   ...
   /**
    ...
    public function removeWhiteListUser($chatroomId,$userId) {
        try{
            ...
            $ret = $this->SendRequest->curl('/chatroom/user/gag/remove.json',$params,'urlencoded','im','POST');
            if(empty($ret))
                throw new \Exception('bad request');
            return $ret;
