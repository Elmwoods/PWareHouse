...
class Bargains extends Base{
	...
	public function uninstall(){
		...
		try{
			...
			Db::name('privileges')->where(["privilegeCode"=>array("like","BARGAIN_%")])->delete();
            //删除微信参数数据
	...
	public function pageQuery(){
		...
		->order('b.startTime asc,b.updateTime desc')
	...
	function helpsList($userId,$bargainUserId){
		...
		if(count($page)>0){
			foreach($page['Rows'] as $key =>$v){
				...
				$page['Rows'][$key]['userPhoto'] = (strpos($userPhoto,'ROOT')!=false)?url('/','','',true).substr($userPhoto,8):$userPhoto;
			}
		}
		return $page;
	}
