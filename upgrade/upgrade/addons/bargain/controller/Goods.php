...
class Goods extends Controller{
	...
	public function weixinUser(){
		$we = WSTWechat();
		$wdata = session('WST_WX_WDATAU');
		if(empty($wdata)){
			$wdata = $we->getUserInfo(input('param.code'));
			session('WST_WX_WDATAU',$wdata);
		}
		...
		if($wdata['openid']!=''){
			//获取标识
			$signinfo = $we->wxUserInfo($wdata['openid']);
	...
	public function wxdetail(){
		...
		if(!empty($goods)){
			...
			//砍价个人信息
			$user = [];
			$user['bargainType'] = $user['userType'] = 0;
			...
			//公众号二维码
			$this->assign('weixinCode',WSTConf('CONF.weixinCode'));
	        $we = WSTWechat();
	        $datawx = $we->getJsSignature('http://'.$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI']);
	        $this->assign("datawx", $datawx);
	        //分享信息
	        $url = urlencode(addon_url('bargain://goods/wxdetail',array('id'=>$bargainId,'bargainUserId'=>base64_encode((int)session('WST_USER.userId'))),true,true));
	        ...
	        $shareInfo['goodsImg'] = WSTRoot()."/".$goods['goodsImg'];
