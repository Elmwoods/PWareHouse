...
class Groupons extends Base{
	...
	public function uninstallMenu(){
		...
		try{
			...
			Db::name('privileges')->where(["privilegeCode"=>array("like","GROUPON_%")])->delete();
            //删除微信参数数据
	...
	public function pageQueryByShop(){
		...
		$where = ['g.shopId'=>$shopId,'gu.dataFlag'=>1];
	...
	public function pageQuery(){
		...
		if(count($page)>0){
			...
			foreach($page['Rows'] as $key =>$v){
				$page['Rows'][$key]['goodsImg'] = WSTImg($v['goodsImg']); 
				$page['Rows'][$key]['zhekou'] = round($v['grouponPrice']/$v['marketPrice']*10,1); 
				if(strtotime($v['startTime'])<=$time && strtotime($v['endTime'])>=$time){
        			$page['Rows'][$key]['status'] = 1; 
        		}else if(strtotime($v['startTime'])>$time){
                    $page['Rows'][$key]['status'] = 0; 
	 ...
	 * 获取团购详情
	 */
	public function getBySale($grouponId){
		$key = input('key');
			...
			if(false !== $result){
				...
				if($order['needPay']==0){
		            ...
		            if((int)WSTConf('CONF.wxenabled')==1){
			            ...
			            WSTWxMessage(['CODE'=>'WX_ORDER_SUBMIT','userId'=>$carts['userId'],'URL'=>Url('wechat/orders/sellerorder','',true,true),'params'=>$params]);
			        }
			    }
			}
			Db::commit();
	...
	*/
	public function illegal(){
		...
		$rs = $this->alias('gu')
		           ...
		           ->where('grouponId',$id)
		           ->field('gu.grouponId,s.userId,g.goodsName,g.goodsSn,gu.grouponStatus,g.goodsId')->find();
		...
		try{
			...
			if($res!==false){
		        ...
		        if((int)WSTConf('CONF.wxenabled')==1){
					...
					WSTWxMessage(['CODE'=>'WX_GROUPON_GOODS_REJECT','userId'=>$rs['userId'],'params'=>$params]);
				}
				Db::commit();
	...
	*/
		...
		try{
			...
			if($res!==false){
		        ...
		        if((int)WSTConf('CONF.wxenabled')==1){
					...
					$params['GOODS_SN'] = $rs['goodsSn'];
					$params['TIME'] = date('Y-m-d H:i:s');          
					WSTWxMessage(['CODE'=>'WX_GROUPON_GOODS_ALLOW','userId'=>$rs['userId'],'params'=>$params]);
				}
				Db::commit();
				return WSTReturn('操作成功',1);
			}
	...
	public function pageQueryByGoods(){
		...
		$shopId = (int)session('WST_USER.shopId');
		$where = ['shopId'=>$shopId,'dataFlag'=>1,'orderCode'=>'groupon','orderCodeTargetId'=>$grouponId];
		if($orderNo!=''){
