	...
	public function add($uId=0){
		...
		try{	
			...
			if($rs !==false){
	            ...
	            $shopUser = Db::name('shops')->where('shopId',$orders['shopId'])->field('userId')->find();
	            if($tpl['tplContent']!=''){
	                $find = ['${ORDER_NO}','${GOODS}'];
	                $replace = [$orders['orderNo'],$orderGoods['goodsName']];
	                WSTSendMsg($shopUser['userId'],str_replace($find,$replace,$tpl['tplContent']),['from'=>6,'dataId'=>$lastId]);
	            }
	            //微信消息
		        if((int)WSTConf('CONF.wxenabled')==1){
	                ...
	                WSTWxMessage(['CODE'=>'WX_ORDER_APPRAISES','userId'=>$shopUser['userId'],'URL'=>'','params'=>$params]);
		        }
				Db::commit();
